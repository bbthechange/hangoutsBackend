...
**Reschedule on direct time edit** (`HangoutServiceImpl.updateHangout`): when a hangout's start time changes via `PUT /hangouts/{id}` and the hangout is still hostless on an in-person watch-party series, the host nudge is re-scheduled (`scheduleHostNudge`'s `createOrUpdate` semantics overwrite the existing schedule in place; no explicit cancel needed). Without this, the EventBridge schedule still points at the *old* fire time — when it fires, the safety window guard (`[now+36h, now+60h]`) in `WatchPartyHostNudgeService` rejects the now-shifted air time, the schedule self-deletes (`ActionAfterCompletion.DELETE`), and the hangout silently loses its host nudge. Sub-day shifts that still land inside the window will fire on the old schedule; the reschedule re-aligns them regardless. Skipped when the hangout already has a host (the abdication branch of `handleWatchPartyHostChange` covers re-arming), when the series is virtual, or when the series isn't a watch party.
