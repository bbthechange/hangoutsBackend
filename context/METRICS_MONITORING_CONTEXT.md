...
- **Watch-party host-nudge schedules** (`WatchPartyHostNudgeScheduler`):
  ...
  - Counter: `watchparty_legacy_model_promoted` (no tags) — increments inside `scheduleHostNudge` whenever a non-null `EventSeries` has `watchPartyModel=null` (legacy row). Such series are treated as IN_PERSON for nudge purposes. Use this to track backfill progress: should trend to zero after legacy series are migrated. Also INFO-logged with the series ID for audit. Note: this fires once per `scheduleHostNudge` call for a legacy series, so a single legacy series with 22 episodes will produce up to 22 increments.
- **Series mute preference batch-get drops** (`SeriesNotificationPreferenceRepositoryImpl.findMutedUsersForSeries`):
  - Counter: `series_pref_batchget_dropped{nudgeType,reason}` — incremented when `BatchGetItem` returns `unprocessedKeys` and we give up. `reason=exhausted` after the retry budget (3 retries, 100/200/400ms backoff + jitter) is used up; `reason=interrupted` if the thread is interrupted mid-backoff (typically JVM shutdown). Each increment counts the muted-preference rows that could not be fetched, so those users may receive a notification they had muted. Alert on any non-zero rate of `reason=exhausted` — it indicates DynamoDB throttle pressure on the series-pref read path and a trust regression for users who silenced a nudge type.
