...
## 6. iOS 2.1.x embedded-vote workaround
...
`PollServiceImpl.transformToPollWithOptionsDTO` (the `GET /hangouts/{id}/polls` list path) never populates the embedded array; default empty list serializes as `"votes":[]` which iOS 2.1 decodes cleanly.

Range chosen as `[2.1.0, 2.2.0)` (not `startsWith("2.1")`) to avoid `2.10.x` collisions and to auto-disable when the iOS fix ships in 2.2 — no backend redeploy required. Predicate fails closed when `appVersion` is null on an iOS client (so we never strip from a client we can't identify).
