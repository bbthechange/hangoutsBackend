...
- **`recomputeMomentum` skips score-based promotion for `null` momentumCategory.** A null category means the hangout was created outside `HangoutServiceImpl.createHangout()` — TV watch party episodes (`WatchPartyServiceImpl` / `WatchPartyBackgroundServiceImpl`), generic series episodes (`EventSeriesServiceImpl`), and legacy pre-momentum rows — none of which run `initializeMomentum`. The read path (`FeedSortingService`) renders null as CONFIRMED, so these already display confirmed; scoring them would spuriously surface GAINING_MOMENTUM and fire a "gaining traction" push on the first RSVP. **The ticket-purchase concrete action (auto-confirm rule 1) still applies to null-category hangouts** — the null guard runs *after* the ticket-purchase check, so a real `TICKET_PURCHASED` participation still promotes to a persisted CONFIRMED and fires the legitimate "bought tickets — it's on!" push. The quiet two-GOING+time+location auto-confirm (rule 2) and BUILDING→GAINING scoring are intentionally skipped for null categories.
